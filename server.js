...
function ensureAdmin(){let db=readDB();if(db.users.some(u=>u.admin))return;let email=(process.env.ADMIN_EMAIL||'admin@fahm.local').toLowerCase(),pw=process.env.ADMIN_PASSWORD||'ChangeMe_123!';let h=hash(pw);db.users.push({id:crypto.randomUUID(),name:'Administrator',email,salt:h.salt,hash:h.hash,status:'approved',admin:true,createdAt:new Date().toISOString()});writeDB(db)}ensureAdmin();
